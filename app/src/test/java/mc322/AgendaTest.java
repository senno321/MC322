...
class AgendaTest {
    ...
    void testAgendarNovaReuniao() {
        assertDoesNotThrow(() -> agenda.agendarNovaReuniao(usuario, "Reunião Importante", "Sala de Reuniões",
                "22/06/2025", "15:00", 60, "Equipe de Devs", "Planejamento Sprint", false));
        assertEquals(1, usuario.getItensAgendados().size());
        assertEquals("Reunião Importante", usuario.getItensAgendados().get(0).getNome());
    }

    /**
     * Testa o agendamento de uma nova atividade vinculada a uma matéria na qual o
     * usuário está inscrito.
     * Verifica se a atividade foi adicionada tanto à agenda do usuário quanto à
     * matéria correta.
     */
    @Test
    void testAgendarNovaAtividade() {
        Materia materia = new Materia("MC322", "Programação Orientada a Objetos", "Professor X", 4);

        Inscricao inscricao = new Inscricao(usuario, materia);
        usuario.getInscricoes().add(inscricao);

        assertDoesNotThrow(() -> agenda.agendarNovaAtividade(usuario, "Prova 1", "Anfiteatro", "25/06/2025", "10:00",
                120, "Programação Orientada a Objetos", 10.0, "Toda a matéria"));
        assertEquals(1, usuario.getItensAgendados().size());
        assertEquals(1, materia.getListaAtividades().size());
        assertEquals("Prova 1", materia.getListaAtividades().get(0).getNome());
    }
    ...
    void testAgendarNovaAtividadeMateriaInexistente() {
        assertThrows(OperationInvalidException.class, () -> agenda.agendarNovaAtividade(usuario, "Prova 1",
                "Anfiteatro", "25/06/2025", "10:00", 120, "MC102", 10.0, "Toda a matéria"));
    }
    ...
    void testAgendarNovoEventoExtensao() {
        assertDoesNotThrow(() -> agenda.agendarNovoEventoExtensao(usuario, "Feira de Ciências", "Ginásio", "30/06/2025",
                "09:00", 480, "Apresentação de projetos", "Comunidade", true, 8));
        assertEquals(1, usuario.getItensAgendados().size());
    }
}